<template>
    <content>
        <!-- 内容开始 -->
        <div class="title">
            <svg t="1705257422086" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
                p-id="1891" width="26px" height="26px">
                <path
                    d="M629.333333 202.666667v213.333333h277.333334v448h-512v-213.333333h-277.333334v-448h512z m213.333334 277.333333h-213.333334v170.666667h-170.666666v149.333333h384v-320z m-277.333334-213.333333h-384v320h213.333334v-170.666667h170.666666v-149.333333z m0 213.333333h-106.666666v106.666667h106.666666v-106.666667z"
                    fill="#000000" p-id="1892"></path>
            </svg> 子网站 / Subsites
        </div>
        <div class="projectList">
            <!-- 项目列表开始 -->
            <!---->
            <a class="projectItem" target="_blank" href="https://www.yangjie.site/record">
                <!-- 项目项开始 -->
                <div class="projectItemLeft">
                    <h1>小杰杂记</h1>
                    <p>之前写得多一些，现在都荒废了。愈发觉得动态网站越用越不喜欢，之后应该会迁到hexo吧</p>
                </div>
                <div class="projectItemRight">
                    <img src="../../assets/images/i1.png" alt="">
                </div>
                <!-- 项目项结束 -->
            </a>
            <!---->
            <a class="projectItem" target="_blank" href="http://ipv6.yangjie.site:5244">
                <!-- 项目项开始 -->
                <div class="projectItemLeft">
                    <h1>Nas管理</h1>
                    <p>绕过nas登录，方便在外面管理，ipv6公网直连</p>
                </div>
                <div class="projectItemRight">
                    <img src="../../assets/images/i2.png" alt="">
                </div>
                <!-- 项目项结束 -->
            </a>
            <!---->
            <a class="projectItem" target="_blank" href="https://www.yangjie.site/nav">
                <!-- 项目项开始 -->
                <div class="projectItemLeft">
                    <h1>小杰导航</h1>
                    <p>做网站的起点，一切的开始</p>
                </div>
                <div class="projectItemRight">
                    <img src="../../assets/images/i3.png" alt="">
                </div>
                <!-- 项目项结束 -->
            </a>
            <!---->
            <a class="projectItem" target="_blank" href="https://www.yangjie.site/foot">
                <!-- 项目项开始 -->
                <div class="projectItemLeft">
                    <h1>小杰足迹</h1>
                    <p>还是想在外面多走走，祖国的大好河山还是看得太少了</p>
                </div>
                <div class="projectItemRight">
                    <img src="../../assets/images/i4.png" alt="">
                </div>
                <!-- 项目项结束 -->
            </a>
            <a class="projectItem" target="_blank" href="https://www.yangjie.site/music">
                <!-- 项目项开始 -->
                <div class="projectItemLeft">
                    <h1>音乐小站</h1>
                    <p>大佬开源的web音乐，我用来放原神的BGM了</p>
                </div>
                <div class="projectItemRight">
                    <img src="../../assets/images/i7.png" alt="">
                </div>
                <!-- 项目项结束 -->
            </a>
            <!---->
        </div>
        <!-- 项目列表结束 -->
        <div class="title">
            <svg t="1705257823317" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
                p-id="7833" width="26px" height="26px">
                <path
                    d="M395.765333 586.570667h-171.733333c-22.421333 0-37.888-22.442667-29.909333-43.381334L364.768 95.274667A32 32 0 0 1 394.666667 74.666667h287.957333c22.72 0 38.208 23.018667 29.632 44.064l-99.36 243.882666h187.050667c27.509333 0 42.186667 32.426667 24.042666 53.098667l-458.602666 522.56c-22.293333 25.408-63.626667 3.392-54.976-29.28l85.354666-322.421333zM416.714667 138.666667L270.453333 522.581333h166.869334a32 32 0 0 1 30.933333 40.181334l-61.130667 230.954666 322.176-367.114666H565.312c-22.72 0-38.208-23.018667-29.632-44.064l99.36-243.882667H416.714667z"
                    fill="#000000" p-id="7834"></path>
            </svg> 技能 / Skills
        </div>
        <div class="skill">
            <!-- 技能开始 -->
            <!-- 前往https://skillicons.dev/
            生成，一个pc端，一个移动端，区别是一行的个数 -->
            <img id="skillPc" src="../../assets/images/skillPc.svg" alt="" srcset="">
            <img id="skillWap" src="../../assets/images/skillWap.svg" alt="" srcset="">
        </div>
        <div class="title">
            <svg class="icon" viewBox="0 0 20 20" height="26px" width="26px">
                <path fill="#000000"
                    d="M18.109,17.776l-3.082-3.081c-0.059-0.059-0.135-0.077-0.211-0.087c1.373-1.38,2.221-3.28,2.221-5.379c0-4.212-3.414-7.626-7.625-7.626c-4.212,0-7.626,3.414-7.626,7.626s3.414,7.627,7.626,7.627c1.918,0,3.665-0.713,5.004-1.882c0.006,0.085,0.033,0.17,0.098,0.234l3.082,3.081c0.143,0.142,0.371,0.142,0.514,0C18.25,18.148,18.25,17.918,18.109,17.776zM9.412,16.13c-3.811,0-6.9-3.089-6.9-6.9c0-3.81,3.089-6.899,6.9-6.899c3.811,0,6.901,3.09,6.901,6.899C16.312,13.041,13.223,16.13,9.412,16.13z">
                </path>
            </svg> 小程序 / 公众号
        </div>
        <img class="xiaochengxu-light-dark-background" src="../../assets/images/2.png" alt="小程序"><img
            class="xiaochengxu-light-dark-background" src="../../assets/images/1.png" alt="公众号"
            style="border-radius: 5px; margin-left: 3px;">

        <!-- 技能结束 -->
    </content>
    <!-- 内容结束 -->
</template>